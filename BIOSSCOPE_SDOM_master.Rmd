...
bioavailability_sum_data <- BIOSSCOPE_sdom %>% 
 group_by(cruise, depth, exudate_condition) %>% 
  ...
  summarize(treatment = c("Control", "SDOM", "SPE-SDOM"),
            ammendment_doc = round(parv_toc[timepoint == 0] - parv_toc[treatment == "Control" & timepoint == 0],0),
            se_ammendment_doc = round(sqrt(parv_toc_sd[timepoint == 0]^2 + parv_toc_sd[treatment == "Control" & timepoint == 0]^2),0),
...
phenolic_content_clean <- phenolic_content %>% 
  ...
  mutate(TPC_HMW_percentDOC = round(TPC_HMW_percentDOC,0),
         sd_TPC_percent_DOC = round(sd_TPC_percent_DOC,0))
...
amendment_bioavailability_table <- bioavailability_sum_phenolic_clean %>% 
  kbl(col.names = c("Physiological State", "Depth Zone", "Treatment","DOC Amended (µMC)", "TPC (%DOC)", "Bioavailability (%)")) %>% 
  kable_styling(html_font = "Times New Roman", bootstrap_options = c("striped","hover", "condensed"), font_size = 24) %>% 
  row_spec(0, bold = T) %>%
...
a270nm_cdom_time_plot <- BIOSSCOPE_cdom_metadata_270 %>% 
   mutate(depth_zone =
...
figure2 <- plot_grid(a270nm_cdom_time_plot, CDOM_T0_TF_plots, nrow = 2, labels = c('A', 'B'), label_fontfamily = "Times New Roman", label_size = 20)
...
ammended_275_plot <- ammended270_recalcitrantDOC %>% 
  ...
  geom_point(size = 6) +
  geom_abline(slope = -5182.3, intercept = 102.06, size = 1) +
...
# Linear Regression between Amendment cocnentration and percent bioavailability
...
# ANOVA for Specific a275 ammendment by treatment, Percent Bioavailability between depths


```{r}

a270_specific_amended_anova <- aov(blanked_abs ~ treatment, data = ammended270_recalcitrantDOC)

summary(a270_specific_amended_anova)

percent_bioavailability_depth <- aov(percent_bioavailable~incubation_zone, data = ammended270_recalcitrantDOC)


summary(percent_bioavailability_depth)
